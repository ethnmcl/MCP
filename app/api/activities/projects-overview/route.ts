...
export async function POST(req:Request){
  try{const body=await req.json();const {sessionid,focus}=zProjectsOverview.parse(body);
    const s=SESSIONS.get(sessionid);
    if(!s?.signedIn) return NextResponse.json({status:'error',message:'unauthorized'},{status:401});
    const projects=[{id:'p1',name:'Onboarding',owner:s.userId,progress:0.5},{id:'p2',name:'Engagement',owner:s.userId,progress:0.2}];
