...
export async function POST(req: Request) {
  try {
    ...
    const { sessionid, villagerID, firstName, lastName, summary } = zPulsesLog.parse(body);
    ...
    if (actingOnBehalf && !(user?.roleName === "Admin" || user?.roleName === "Coach")) {
      return NextResponse.json({ status: "error", message: "forbidden_on_behalf" }, { status: 403 });
    }
    ...
    list.unshift(pulse);
    PULSES.set(sessionid, list.slice(0, 100));
