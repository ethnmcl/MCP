...
export async function POST(req:Request){
  try{const body=await req.json();const {sessionid,phone,code}=zVerifyOtp.parse(body);
    const issue=OTPS.get(phone);
    if(!issue) return NextResponse.json({status:'error',message:'otp_not_found'},{status:400});
    if(issue.sessionid!==sessionid) return NextResponse.json({status:'error',code:-32041,message:'session_binding_mismatch'},{status:409});
    if(now()>issue.expiresAt) return NextResponse.json({status:'error',message:'otp_expired'},{status:400});
    if(issue.code!==code){issue.attempts++;return NextResponse.json({status:'error',message:'otp_invalid',attempts:issue.attempts},{status:400});}
    const user={userId:'u_'+phone.replace(/\D/g,''),email:`{${}}`,name:'Villager',avatar:null,status:'active',roleId:'role_user',roleName:'User'};
    user.email = `${phone.replace(/\D/g,'')}@example.test`;
